...
const events = ['pd-expand', 'pd-edit', 'pd-selected', 'pd-collapsed', 'pd-content-click'];
addEventlisteners('pd-list', events);
...
type ExpandableArgs = typeof expandableArgs;

const expandableArgTypes: ArgTypes = {
    status: { control: { type: 'select' }, options: ['-none-', 'success', 'danger', 'warning', 'info', 'unset'] },
};

export const ExpandableList: StoryObj<ExpandableArgs> = {
