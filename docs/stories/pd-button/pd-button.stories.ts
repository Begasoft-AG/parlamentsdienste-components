...
<pd-button class="mt-3 ml-3" ${args.outline ? 'outline' : ''} ${args.disabled ? 'disabled' : ''} size=${
    args.size
} type="${args.type}" full-width="${args.fullWidth}" ${args.showAsLink ? 'show-as-link' : ''}>Primary</pd-button>
<pd-button class="mt-3 ml-3" ${args.outline ? 'outline' : ''} ${args.disabled ? 'disabled' : ''} size=${
    args.size
} color="success" type="${args.type}" full-width="${args.fullWidth}" ${
    args.showAsLink ? 'show-as-link' : ''
}>Success</pd-button>
<pd-button class="mt-3 ml-3" ${args.outline ? 'outline' : ''} ${args.disabled ? 'disabled' : ''} size=${
    args.size
} color="danger" type="${args.type}" full-width="${args.fullWidth}" ${
    args.showAsLink ? 'show-as-link' : ''
}>Danger</pd-button>
<pd-button class="mt-3 ml-3" ${args.outline ? 'outline' : ''} ${args.disabled ? 'disabled' : ''} size=${
    args.size
} color="warning" type="${args.type}" full-width="${args.fullWidth}" ${
    args.showAsLink ? 'show-as-link' : ''
}>Warning</pd-button>
<pd-button class="mt-3 ml-3" ${args.outline ? 'outline' : ''} ${args.disabled ? 'disabled' : ''} size=${
    args.size
} color="info" type="${args.type}" full-width="${args.fullWidth}" ${
    args.showAsLink ? 'show-as-link' : ''
}>Info</pd-button>
<pd-button class="mt-3 ml-3" ${args.outline ? 'outline' : ''} ${args.disabled ? 'disabled' : ''} size=${
    args.size
} color="light" type="${args.type}" full-width="${args.fullWidth}" ${
    args.showAsLink ? 'show-as-link' : ''
}>Light</pd-button>
<pd-button class="mt-3 ml-3" ${args.outline ? 'outline' : ''} ${args.disabled ? 'disabled' : ''} size=${
    args.size
} color="dark" type="${args.type}" full-width="${args.fullWidth}" ${
    args.showAsLink ? 'show-as-link' : ''
}>Dark</pd-button>
`;
...
const iconArgTypes: ArgTypes = {
    iconLocation: { control: { type: 'select' }, options: ['left', 'right', 'center'] },
};
type IconArgs = { iconLocation: string };
...
const iconButtons = args => `
    ...
    <div>
        <pd-button class="mt-3 ml-3" icon-location="${
            args.location
        }" href="http://www.google.ch" target="_blank" full-width="${args.fullWidth}">
...
export const Icon: StoryObj<ButtonArgs & IconArgs> = {
    render: iconButtons,
    args: { iconLocation: 'left' },
...
export const Link: StoryObj<ButtonArgs & LinkArgs> = {
    render: linkButton,
    args: linkArgs,
...
export const Group: StoryObj<ButtonArgs & GroupArgs> = {
    render: groupStory,
    args: groupArgs,
};
