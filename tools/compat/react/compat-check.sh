...
log_step "Scaffolding fresh React ${REACT_VERSION} consumer with create-vite ${create_vite_version}"
npm create "vite@${create_vite_version}" "$project_name" -- --template react-ts
...
npm install --no-audit --no-fund \
    ...
    "$(compgen -G "$prebuilt_pack_dir/react/parlamentsdienste-pdcomponents-react-*.tgz" | head -n 1)"
