...
test('pd-dropdown', async ({ page }) => {
    ...
    const option = await page.locator('pd-dropdown-item').nth(2);
    await option.click();
    await page.waitForTimeout(500);
...
test('pd-combobox selectable', async ({ page }) => {
    ...
    await page.waitForTimeout(500);
    const option = await page.locator('[data-test="pd-combobox-selectable"] pd-dropdown-item').nth(0);
    await option.click();
    await page.waitForTimeout(500);
...
test('pd-combobox multiselect', async ({ page }) => {
    ...
    const option1 = await page.locator('[data-test="pd-combobox-multiselect"] pd-dropdown-item').nth(1);
    const option2 = await page.locator('[data-test="pd-combobox-multiselect"] pd-dropdown-item').nth(3);
    const option3 = await page.locator('[data-test="pd-combobox-multiselect"] pd-dropdown-item').nth(4);
    await option1.click();
    await option2.click();
    await option3.click();
    await page.waitForTimeout(500);

    const values = comboboxMultiselectValue as Array<{ label: string }>;
    expect(Array.isArray(values)).toBe(true);
    expect(values.length).toBeGreaterThanOrEqual(2);
    expect(values[0].label).toBe('Pa.Iv. Semadeni. Fakultatives');
    expect(values[1].label).toBe('Obligatorisches');
    expect(values[2].label).toBe('Anfrage');
});
