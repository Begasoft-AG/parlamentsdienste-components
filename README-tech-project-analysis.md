...
flowchart TD
    ...
    D --> E[packages/angular/src/lib/angular/components.ts Barrel]
    D --> EA[packages/angular/src/lib/angular/pd-*.ts]
...
- Der Output Target nutzt fuer `dist-custom-elements` `esModules: true`. Dadurch ist [packages/angular/src/lib/angular/components.ts](../../packages/angular/src/lib/angular/components.ts) nur noch eine Barrel-Datei, waehrend pro Web Component eine eigene Proxy-Datei `packages/angular/src/lib/angular/pd-*.ts` erzeugt wird.
...
[packages/core/package.json](../../packages/core/package.json) definiert die operative Quelle des Systems. `build` fuehrt `stencil build` aus und patched danach den generierten Custom-Elements-Einstieg ueber [packages/core/scripts/patch-components-index.mjs](../../packages/core/scripts/patch-components-index.mjs). `start` startet den Watch-Server, `test` fuehrt Spec- und E2E-Tests der Components aus.

Der Postbuild-Patch in [packages/core/scripts/patch-components-index.mjs](../../packages/core/scripts/patch-components-index.mjs) entfernt aus `packages/core/components/index.js` eine von Stencil erzeugte Lazy-Loader-Fallback-Importstelle auf `*.entry.js`. Im `dist-custom-elements`-Output liegen die direkt importierten Komponenten aber als `p-*.js` beziehungsweise `pd-*.js` Module vor. Angular/esbuild interpretiert den Fallback sonst als dynamisches Glob-Importmuster und meldet eine `empty-glob`-Warnung. Das Script ersetzt diesen ungenutzten Fallback durch einen explizit fehlschlagenden `Promise.reject(...)`, damit der Custom-Elements-Build keine nicht vorhandenen Entry-Dateien referenziert.
...
| `angularOutputTarget()` | Angular-Proxies, Barrel-Datei, Runtime-Helfer | `packages/angular/src/lib/angular/components.ts`, `packages/angular/src/lib/angular/pd-*.ts`, `packages/angular/src/lib/generated/*` |
...
| --------- | -------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| generiert | [packages/angular/src/lib/angular/components.ts](../../packages/angular/src/lib/angular/components.ts)                     | Barrel fuer alle per `esModules` erzeugten Proxy-Dateien                                  |
| generiert | `packages/angular/src/lib/angular/pd-*.ts`                                                                                 | einzelne Standalone-Proxies, inklusive `ValueAccessor`-Vererbung und Output-Event-Mapping |
...
Der entscheidende Punkt fuer die Analyse: Angular wird nicht nur aus eigenen Quellen gebaut, sondern aus einer Mischung aus manuell gepflegtem Runtime-Code und waehrend des Core-Builds in dieses Paket hineingeschriebenen Dateien. Mit `esModules: true` gehoeren auch die generierten `pd-*.ts` Proxy-Dateien zu den versionierten Angular-Quellen; das npm-Paket selbst enthaelt sie nicht einzeln, sondern das von `ng-packagr` daraus erzeugte FESM-Bundle und die Deklarationen.
...
| ----------------------------------------- | ----------------------- | --------------------------------------------------------------------------------------------- |
| `@parlamentsdienste/pdcomponents-core`    | `packages/core`         | erfolgreicher `stencil build` inklusive `components/index.js`-Patch                           |
| `@parlamentsdienste/pdcomponents-angular` | `packages/angular/dist` | Core-Build hat `components.ts`, `pd-*.ts` und Runtime-Helfer geschrieben, danach `ng-packagr` |
...
| -------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- | ------------------------------------------------------------------------------- |
| [packages/angular/src/lib/angular/components.ts](../../packages/angular/src/lib/angular/components.ts)                                       | generiert         | `angularOutputTarget()` waehrend `build:core`, Barrel bei `esModules`           |
| `packages/angular/src/lib/angular/pd-*.ts`                                                                                                   | generiert         | `angularOutputTarget()` waehrend `build:core`, einzelne Proxies bei `esModules` |
